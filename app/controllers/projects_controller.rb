    ...
    if params[:page]
      if params[:user_id]
        ...
        if (position) < projects.length
          render json: projects.slice(position, 4), each_serializer: ProjectShowSerializer, include: ["model_groups", "model_groups.child_groups", "model_groups.model_planes", "model_groups.model_boxes", "model_groups.model_spheres"]
        else
        ...
        if (position) < projects.length
          render json: projects.slice(position, 6), each_serializer: ProjectShowSerializer, include: ["model_groups", "model_groups.child_groups", "model_groups.model_planes", "model_groups.model_boxes", "model_groups.model_spheres"]
        else
          render json: {error: "page number excess searchable pages "}, status: 405
        end
      end
